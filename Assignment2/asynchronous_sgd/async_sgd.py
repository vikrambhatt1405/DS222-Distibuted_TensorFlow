...
def main(_):
  ...
  if FLAGS.job_name == "ps":
    ...
    with tf.device(tf.train.replica_device_setter(
      ...
      summary_hook = tf.train.SummarySaverHook(save_steps=5,output_dir="/home/vikrambhatt/tf_summary_logs_worker{}".format(FLAGS.task_index),summary_op=merged)
      checkpoint_hook = tf.train.CheckpointSaverHook(save_steps=5,checkpoint_dir="/home/vikrambhatt/tf_checkpoint_logs_worker{}".format(FLAGS.task_index),saver=saver)
      hooks=[tf.train.StopAtStepHook(last_step=1000000),summary_hook,checkpoint_hook]
      train_writer = tf.summary.FileWriter(logdir+"tf_summary_logs_worker{}/train".format(FLAGS.task_index), tf.get_default_graph())
      test_writer=tf.summary.FileWriter(logdir+"tf_summary_logs_worker{}/test".format(FLAGS.task_index))
      devel_writer=tf.summary.FileWriter(logdir+"tf_summary_logs_worker{}/devel".format(FLAGS.task_index))
